...
class AlexReportTests: XCTestCase {
    
    private var results = [Any]()
    private var mainController: MainViewController!
    
    struct Constants {
        static let sitesFile = "top500.json"
        static let reportFile = "top500-grades_\(AlexReportTests.dateString()).json"
    }
    ...
    func testTopSites() {
        let data = try! FileLoader().load(fileName: Constants.sitesFile, fromBundle: Bundle(for: AlexReportTests.self))

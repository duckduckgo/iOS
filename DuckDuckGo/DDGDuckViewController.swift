...
//    self.reloadFavorites()
  ...
  func reloadSuggestions() {
    let searchText = self.filterString
    let suggester = DDGSearchSuggestionsProvider.shared()
    ...
    suggester.downloadSuggestions(forSearchText: searchText, success: { () -> Void in
      DispatchQueue.main.async(execute: { () -> Void in
        if self.filterString.isEqual(searchText) {
          weakSelf?.suggestions = suggester.suggestions(forSearchText: self.filterString)
  ...
  override func viewDidLoad() {
    ...
    let tableView = UITableView(frame: viewFrame, style: .grouped)
    tableView.autoresizingMask = [.flexibleHeight, .flexibleWidth]
    tableView.delegate = self
    tableView.dataSource = self
    tableView.sectionFooterHeight = 0.01
    tableView.backgroundColor = UIColor.duckStoriesBackground
  ...
  func showSuggestions(_ suggestions: [Dictionary<String, String>]) {
    self.imageRequestQueue?.cancelAllOperations()
...
//    } else if indexPath.section == SUGGESTION_SECTION {
      ...
      let searchField = self.duckSearchController?.searchBar?.searchField
...
//    self.hideKeyboardAfterDelay()
  ...
  func hideKeyboardAfterDelay() {
    ...
    let popTime = DispatchTime(uptimeNanoseconds: DispatchTime.now().uptimeNanoseconds + NSEC_PER_SEC/2)  // + Double(0.4 * Double(NSEC_PER_SEC))

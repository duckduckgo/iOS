...
class DDGSearchBar : UIView {
  ...
  func showBangButton(show:Bool=true, animated:Bool=false) {
  ...
  func showCancelButton(show:Bool=true, animated:Bool=false) {
  ...
  func showLeftButton(show:Bool=true, animated:Bool=false) {
    self.isShowsLeftButton = show
    guard let leftButton=self.leftButton, let leftConstraint = self.leftButtonXConstraint else {
      return
    }
    
    leftConstraint.constant = show ? leftButton.frame.size.width + 10 : 0
    leftButton.alpha = show ? 1 : 0
  ...
  func setProgress(_ newProgress: Double) {
    self.progressView?.percentCompleted = CGFloat(Double.maximum(0, Double.minimum(ceil(newProgress * 100.0), 100.0)))
  }

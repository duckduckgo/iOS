...
struct VideoPlayerView: View {
    ...
    init(
        ...
        loopVideo: Bool = false
         ...
         let playerModel = VideoPlayerViewModel(url: url, autoPlay: autoPlay, loopVideo: loopVideo)
        _model = StateObject(wrappedValue: playerModel)
    }

    var body: some View {
        VideoPlayer(player: model.player)
            .disabled(false) // Disable player controls
            .onFirstAppear {
                if model.autoPlay {
                    model.play()
                }
...
final class VideoPlayerViewModel: ObservableObject {
    ...
    init(url: URL, autoPlay: Bool, loopVideo: Bool) {
        self.url = url
